<?php
// Booking request (casting call) — multi-column layout + shared header shell
	...
	try {
		...
		if ($user) {
			...
			if ($draft['contact_email'] === '') {
				throw new Exception('Email is required so the artist can reply.');
			}
		}
...
<style>
/* Request form: 3 cols desktop, 2 cols tablet, 1 col mobile */
.request-form-grid{
  display:grid;
...
.request-form-grid .span-2{ grid-column: span 2; }
.request-form-grid .span-3{ grid-column: span 3; }

@media (max-width: 980px){
  .request-form-grid{ grid-template-columns:repeat(2, minmax(0,1fr)); }
  .request-form-grid .span-3{ grid-column: span 2; }
}
@media (max-width: 640px){
  .request-form-grid{ grid-template-columns:1fr; }
  .request-form-grid .span-2,
  .request-form-grid .span-3{ grid-column: span 1; }
}
</style>

<div style="max-width: 920px; margin: 0 auto; padding: 1.25rem;">
	<div class="card">
		<div class="card-body">
			...
			<form method="post" class="form">
				...
				<div class="request-form-grid">
					...
					<div class="span-3" style="display:flex; gap:.75rem; align-items:center;">
						...
						<div>
							...
							<select name="requester_profile_id">
								...
								<?php foreach ($myProfiles as $p): ?>
									<option value="<?= (int)$p['id'] ?>" <?= ((int)$draft['requester_profile_id']===(int)$p['id'])?'selected':'' ?>>
										<?= h2($p['name']) ?> (<?= h2($p['profile_type']) ?>)
									</option>
								<?php endforeach; ?>
							</select>
						</div>
